...
{
	nmea::osd osd;
	osd.set_course(123.4, nmea::reference::TRUE);
...
{
	nmea::osd osd;
	osd.set_speed(5.4, nmea::reference::TRUE);

	EXPECT_STREQ("$IIOSD,,,,,5.4,T,,,*0F", nmea::to_string(osd).c_str());
}

TEST_F(Test_nmea_osd, set_drift)
{
	nmea::osd osd;
	osd.set_drift(1.2, nmea::unit::KNOT);
