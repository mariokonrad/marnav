...
using manufacturer_map = std::map<std::string, entry>;

#define MANUFACTURER(tag, id, text)        \
	{                                      \
		tag, { manufacturer_id::id, text } \
	}

// clang-format off
static const manufacturer_map manufacturer_table = {
	MANUFACTURER("3SN", _3SN, "3-S NAVIGATION"),
...
{
	const auto it = manufacturer_table.find(tag);
	if (it == manufacturer_table.cend())
		return manufacturer_table.cend();
	return it;
}
