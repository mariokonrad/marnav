...
void gtd::check_index(int index) const throw(std::out_of_range)
{
...
double gtd::get_time_diff(int index) const throw(std::out_of_range)
{
	check_index(index);
	return time_diffs[index];
}

void gtd::set_time_diff(int index, double value) throw(std::out_of_range)
{
	check_index(index);
	time_diffs[index] = value;
}

std::unique_ptr<sentence> gtd::parse(const std::string & talker,
