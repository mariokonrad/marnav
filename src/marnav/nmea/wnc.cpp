...
wnc::wnc()
	: sentence(ID, "WNC", talker_id::global_positioning_system)
{
}
...
{
	distance_nm = t;
	distance_nm_unit = unit::NM;
}

void wnc::set_distance_km(double t)
{
	distance_km = t;
	distance_km_unit = unit::KM;
}
...
void wnc::check_waypoint_id(const std::string & id) const throw(std::invalid_argument)
{
	if (id.size() > 8)
