...
{
	EXPECT_ANY_THROW(ais::message_04::parse(ais::raw{167}));
	EXPECT_ANY_THROW(ais::message_04::parse(ais::raw{169}));
}
...
TEST_F(Test_ais_message_04, set_latitude)
{
	...
	ASSERT_EQ(1u, v.size());
	EXPECT_STREQ("4000000000Htt<t[F00e<<000000", v[0].first.c_str());
	EXPECT_EQ(0, v[0].second);
}
...
{
	std::vector<std::pair<std::string, int>> v;
	v.push_back(std::make_pair("4000000000Htt<t[F00e<<000000", 0));
...
TEST_F(Test_ais_message_04, set_longitude)
{
	...
	ASSERT_EQ(1u, v.size());
	EXPECT_STREQ("4000000000Htt0pPdhl4Y@000000", v[0].first.c_str());
	EXPECT_EQ(0, v[0].second);
}
...
{
	std::vector<std::pair<std::string, int>> v;
	v.push_back(std::make_pair("4000000000Htt0pPdhl4Y@000000", 0));
