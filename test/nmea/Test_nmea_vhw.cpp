...
TEST_F(Test_nmea_vhw, properties) { nmea_sentence_traits<nmea::vhw>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::vhw>(nmea::talker_id::none, {7, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::vhw>(nmea::talker_id::none, {9, "@"}));
}
