...
TEST_F(Test_nmea_lcd, contruction) { EXPECT_NO_THROW(nmea::lcd lcd); }

TEST_F(Test_nmea_lcd, properties) { nmea_sentence_traits<nmea::lcd>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::lcd>(nmea::talker_id::none, {12, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::lcd>(nmea::talker_id::none, {14, "@"}));
}
