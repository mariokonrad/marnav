...
TEST_F(Test_nmea_gbs, properties) { nmea_sentence_traits<nmea::gbs>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::gbs>(nmea::talker_id::none, {7, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::gbs>(nmea::talker_id::none, {9, "@"}));
}
