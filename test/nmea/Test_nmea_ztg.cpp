...
TEST_F(Test_nmea_ztg, contruction) { EXPECT_NO_THROW(nmea::ztg ztg); }

TEST_F(Test_nmea_ztg, properties) { nmea_sentence_traits<nmea::ztg>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::ztg>(nmea::talker_id::none, {2, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::ztg>(nmea::talker_id::none, {4, "@"}));
}
