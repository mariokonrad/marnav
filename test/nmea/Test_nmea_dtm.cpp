...
TEST_F(Test_nmea_dtm, properties) { nmea_sentence_traits<nmea::dtm>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::dtm>(nmea::talker_id::none, {7, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::dtm>(nmea::talker_id::none, {9, "@"}));
}
