...
TEST_F(Test_nmea_vwr, properties) { nmea_sentence_traits<nmea::vwr>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::vwr>(nmea::talker_id::none, {7, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::vwr>(nmea::talker_id::none, {9, "@"}));
}
