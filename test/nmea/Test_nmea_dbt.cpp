...
TEST_F(Test_nmea_dbt, properties) { nmea_sentence_traits<nmea::dbt>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::dbt>("@@", {5, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::dbt>("@@", {7, "@"}));
}
