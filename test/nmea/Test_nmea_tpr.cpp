...
TEST_F(Test_nmea_tpr, properties) { nmea_sentence_traits<nmea::tpr>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::tpr>(nmea::talker_id::none, {5, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::tpr>(nmea::talker_id::none, {7, "@"}));
}
