...
TEST_F(Test_nmea_gtd, properties) { nmea_sentence_traits<nmea::gtd>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::gtd>(nmea::talker_id::none, {4, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::gtd>(nmea::talker_id::none, {6, "@"}));
}
