...
TEST_F(Test_nmea_gns, contruction) { EXPECT_NO_THROW(nmea::gns gns); }

TEST_F(Test_nmea_gns, properties) { nmea_sentence_traits<nmea::gns>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::gns>(nmea::talker_id::none, {11, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::gns>(nmea::talker_id::none, {13, "@"}));
}
