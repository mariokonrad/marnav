...
TEST_F(Test_nmea_hfb, properties) { nmea_sentence_traits<nmea::hfb>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::hfb>(nmea::talker_id::none, {3, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::hfb>(nmea::talker_id::none, {5, "@"}));
}
