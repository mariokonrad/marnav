...
TEST_F(Test_nmea_wcv, properties) { nmea_sentence_traits<nmea::wcv>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::wcv>(nmea::talker_id::none, {2, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::wcv>(nmea::talker_id::none, {4, "@"}));
}
