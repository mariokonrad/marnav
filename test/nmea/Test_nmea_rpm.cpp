...
TEST_F(Test_nmea_rpm, contruction)
{
	EXPECT_NO_THROW(nmea::rpm rpm);
}

TEST_F(Test_nmea_rpm, properties)
{
	nmea_sentence_traits<nmea::rpm>();
}
...
{
	EXPECT_ANY_THROW(
		nmea::detail::factory::sentence_parse<nmea::rpm>(nmea::talker_id::none, {4, "@"}));
	EXPECT_ANY_THROW(
		nmea::detail::factory::sentence_parse<nmea::rpm>(nmea::talker_id::none, {6, "@"}));
}
