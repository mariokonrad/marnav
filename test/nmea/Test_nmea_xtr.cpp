...
TEST_F(Test_nmea_xtr, contruction) { EXPECT_NO_THROW(nmea::xtr xtr); }

TEST_F(Test_nmea_xtr, properties) { nmea_sentence_traits<nmea::xtr>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::xtr>(nmea::talker_id::none, {2, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::xtr>(nmea::talker_id::none, {4, "@"}));
}
