...
TEST_F(Test_nmea_zfo, properties) { nmea_sentence_traits<nmea::zfo>(); }
...
{
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::zfo>(nmea::talker_id::none, {2, "@"}));
	EXPECT_ANY_THROW(nmea::sentence_parse<nmea::zfo>(nmea::talker_id::none, {4, "@"}));
}
